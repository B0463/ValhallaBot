...
function exec(Bot){
    if(config.getCache("timers.instagram.lastId")) {
        Bot.channels.cache.get(config.get("timers.instagram.chatId"))
    ...
    Bot.channels.cache.get(config.get("timers.instagram.chatId")).send({ embeds: [embed] }).then((msg) => {config.saveCache("timers.instagram.lastId", msg.id)});
}
export default {
    exec
};